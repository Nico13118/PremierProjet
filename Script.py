...
os.mkdir("C:\Projets\PremierProjet\BooksToScrap")
...
while h < nombre_categorie_accueil1:
    ...
    if h == nombre_categorie_accueil1:
        ...
        os.mkdir(f"C:\Projets\PremierProjet\BooksToScrap\ {nom_categorie_accueil}")
        i = i + 1
...
nombre_de_categorie = len(liste_categorie_accueil2)
#Création d'une boucle qui va lister les liens de catégorie une par une
f, g = 0, 0
nombre_de_categorie = nombre_de_categorie + 1
while g < nombre_de_categorie:
    g = g + 1
    if g == nombre_de_categorie:
        break
    elif g < nombre_de_categorie:
                ...
                if c < nombre_page:
                    ...
                    while d < resultat_livre:
                        ...
                        if e < resultat_livre:
                            ...
                            print(f"Enregistrement des informations du livre : {title} dans un fichier au format csv dans le répertoire : {category2} \nEmplacement du fichier csv : C:\Projets\PremierProjet\BooksToScrap")
                            print()
                            # Création de l'en-tête
                            en_tete = ["product_page_url", "universal_product_code(upc)", "title", "price_including_tax",
                                       "price_excluding_tax", "number_available", "product_description", "category",
                                       "review_rating", "image_url"]
                            if os.path.exists(f"C:\Projets\PremierProjet\BooksToScrap\ {category2}\output.csv"):
                                print()
                            else:
                                with open(f"C:\Projets\PremierProjet\BooksToScrap\ {category2}\output.csv", "w", encoding="utf-8") as fichier_csv:
                                    writer = csv.writer(fichier_csv, delimiter=";")
                                    writer.writerow(en_tete)

                            with open(f"C:\Projets\PremierProjet\BooksToScrap\ {category2}\output.csv", "a", encoding="utf8") as fichier_csv:
                                writer = csv.writer(fichier_csv, delimiter=";")
